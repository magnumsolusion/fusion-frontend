...
const handleOpenEdit = async (id) => {
  try {
    ...
    const { product_name, product_code, type, category_id, vendor_id, descriptions } = response.data;
...
const columns = [
  ...
  { field: 'product_code', headerName: 'Code', width: 250 },
  ...
  { field: 'descriptions', headerName: 'Descriptions', width: 150},
...
const columns2 = [
  ...
  { field: 'end_price', headerName: 'End Price', width: 200},
...
const handleCloseModalEdit = () => {
  ...
  const handleSubmitAdd2 = async (e) => {
    ...
    if (!selectedOptionUnit || !startstock || !endstock || !maxstock || !minstock || !startprice || !endprice || !lastbuyprice) {
  ...
  const handleEditCustomerContact = async (id) => {
    try {
      ...
      const { unit_id, start_stock, end_stock, start_price, end_price, max_stock, min_stock, last_buy_price, enable, allow_buy, allow_sell } = response.data;
     console.log(response.data);
     setShowForm(true);
     setShowButton(true);
