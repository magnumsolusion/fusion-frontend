...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/getUnits`, {
        headers: {
...
const handleSubmitAdd2 = async (e) => {
  ...
  try {
    ...
    const response = await axios.post(`${apiUrl}/api/registerUnits`,formData ,{
      headers: {
...
const handleUpdateProductCategory = async (idProductCategory) => {
  try {
    ...
    const response = await axios.patch(`${apiUrl}/api/updateUnits/${idProductCategory}`,formData ,{
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const handleEditProductCategory = async (id) => {
  try {
    // Make PATCH request to update vendor data
    const response = await axios.get(`${apiUrl}/api/getUnitsById/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
   ...
   console.log(response.data);
   setUnitsName(response.data.units_name)
  ...
  const handleDeleteConfirmation = async () => {
    try {
      const response = await axios.delete(`${apiUrl}/api/deleteUnits/${deletingId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setDeletingUnitsName(response.data.units_name);
  ...
  const columns = [
    ...
    { field: 'units_name', headerName: 'Name', width: 250 },
