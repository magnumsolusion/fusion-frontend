...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/getDivision`, {
        headers: {
...
const handleSubmitAdd2 = async (e) => {
  ...
  try {
    const formData = {
      ...
      division_name: unitsname,
      //user_created:uid,

    };
    const response = await axios.post(`${apiUrl}/api/registerDivision`,formData ,{
      headers: {
...
const handleUpdateProductCategory = async (idProductCategory) => {
  try {
    ...
    const response = await axios.patch(`${apiUrl}/api/updateDivision/${idProductCategory}`,formData ,{
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const handleEditProductCategory = async (id) => {
  try {
    // Make PATCH request to update vendor data
    const response = await axios.get(`${apiUrl}/api/getDivisionById/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
   ...
   console.log(response.data);
   setUnitsName(response.data.division_name)

   setIdProductCategory(response.data.division_id)
   setUpdating(true);
  ...
  const handleDeleteConfirmation = async () => {
    try {
      const response = await axios.delete(`${apiUrl}/api/deleteDivision/${deletingId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setDeletingUnitsName(response.data.division_name);
  ...
  const columns = [
    ...
    { field: 'division_name', headerName: 'Name', width: 250 },
  ...
  
  {
    ...
    renderCell: (params) => (
      <>
       <button className='edit-button' type='button' onClick={() => handleEditProductCategory(params.row.division_id)}>Edit</button>&nbsp;
       <button type='button'
              onClick={() => {
                setDeletingId(params.row.division_id);
