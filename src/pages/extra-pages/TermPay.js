...
const TermPay = () => {
  const mainCardTitle = "Terms Payment";
  //for add
  ...
  const [termpayname, setTermPayName] = useState('');
  const [days, setDays] = useState('');
  const[idProductCategory, setIdProductCategory] = useState(null);

  const [rowsTermPay, setTermPay] = useState([]);
...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/getTermPay`, {
        headers: {
          Authorization: `Bearer ${token}`, // Add your token if needed
        },
      });
      setTermPay(response.data); 
...
}, [token, rowsTermPay]); // Add token as a dependency if it's required
...
const handleSubmitAdd2 = async (e) => {
  ...
  try {
    const formData = {
      ...
      termpay_name: termpayname,
      days: days,
      user_created:uid,

    };
    const response = await axios.post(`${apiUrl}/api/registerTermPay`,formData ,{
      headers: {
...
const handleUpdateProductCategory = async (idProductCategory) => {
  try {
    ...
    const response = await axios.patch(`${apiUrl}/api/updateTermPay/${idProductCategory}`,formData ,{
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const handleEditProductCategory = async (id) => {
  try {
    // Make PATCH request to update vendor data
    const response = await axios.get(`${apiUrl}/api/getTermPayById/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
   ...
   console.log(response.data);
   setTermPayName(response.data.termpay_name)
  ...
  const handleDeleteConfirmation = async () => {
    try {
      const response = await axios.delete(`${apiUrl}/api/deleteTermPay/${deletingId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setTermPay((prevRows) => prevRows.filter((row) => row.id !== deletingId));
      setDeletingUnitsName(response.data.termpay_name);
  ...
  const columns = [
    ...
    { field: 'termpay_name', headerName: 'Name', width: 250 },
     ...
     <>
        <button onClick={handleSubmitAdd2} className='add-button'>Add</button>
                                      ...
                                      <input className='select-element'
                                        ...
                                        value={termpayname}
                                        name="termpayname"
                                        onChange={(e) => setTermPayName(e.target.value)}
