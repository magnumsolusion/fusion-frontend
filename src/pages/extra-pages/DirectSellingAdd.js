...
Object.keys(barcodeData).forEach(async (productId, index) => {
  barcodeData[productId].forEach(async (barcode) => {
    try {
      ...
      const responseBarcode = await axios.patch(
        ...
        { reference_id: NewId, user_updated: uid, reference_status:'SL', out_code:sellingCode, reference_id_product:ProductNewIds[index] }
      );
 ...
 const [barcodeID, setBarcodeID] = useState(null);
  ...
  const handleAddBarcode = (e) => {
    ...
    if (barcodeAvailability) {
     ...
     setSuccessMessageAdd('Barcode is not available. Please choose a different barcode.');
    }
  };
