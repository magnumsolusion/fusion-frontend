...
const DirectBuying = () => {
  const mainCardTitle = "Direct Buying";
  //for add
...
//AUTH TOKEN
  ...
  const fetchVendorName = async (id) => {
    try {
      ...
      setDeletingVendorName(response.data[0].buying_code);
...
const columns = [
  ...
  { field: 'buying_code', headerName: 'Buying Code', width: 150 },
  { field: 'purchase_number', headerName: 'Nota Number', width: 150 },
  { field: 'purchase_date', headerName: 'Date', width: 250 },
...
// ACTION GROUP FUNCTION
  ...
  const handleDeleteConfirmation = async () => {
    try {
      await axios.delete(`${apiUrl}/api/deleteDirectBuying/${deletingId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
