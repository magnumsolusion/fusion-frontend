...
}, [token, rows]); // Add token as a dependency if it's required
    ...
    if (editingId) {
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          setRowsContact([]); // Set rowsContact to an empty array when no data is available
        }
      } catch (error) {
        console.error('Error fetching project data:', error);
        setRowsContact([]); 
...
const handleCloseModalEdit = () => {
  ...
  const handleSubmitAdd2 = async (e) => {
    ...
    try {
      ...
      // Fetch updated vendor data after successful submission
      const response = await axios.get(`${apiUrl}/api/getCustomerContact`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
